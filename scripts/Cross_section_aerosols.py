...
u_vals = np.asarray([S0.variables['u'][:height],S1.variables['u'][:height],R0.variables['u'][:height],R1.variables['u'][:height]])
v_vals = np.asarray([S0.variables['v'][:height],S1.variables['v'][:height],R0.variables['v'][:height],R1.variables['v'][:height]])
w_vals = np.asarray([S0.variables['w'][:height],S1.variables['w'][:height],R0.variables['w'][:height],R1.variables['w'][:height]])
...
titles = ['S0','S1','R0','R1']
titles = ['\mathrm{R_0A_0}','\mathrm{R_0A_1}', '\mathrm{R_1A_0}', '\mathrm{R_1A_1}','\mathrm{R_1A_1}*']
...
if (plot=='cross'):
    ...
    for i,ax in enumerate(axes.flatten()):
            ...
            w = w_vals[i]
            Ntot = Ntot_vals[i]*1E-6
            Ntot[:8][idx3[:8]] = np.nan
            ...
            c = ax.streamplot(x,z,u,w,density=0.5,color='k')
            c.lines.set_alpha(0.5)
            ...
            if (i==0):
                ...
                im = ax.imshow(Ntot,origin='lower',cmap=current_cmap,norm=mpl.colors.LogNorm(vmin=1E4,vmax=9E4))
                base = Ntot
                ...
                # delta[np.isnan(base)] = np.nan
                cmap2 = plt.cm.RdBu_r
                cmap2.set_bad('black')
                print ('Decrease in canyon: {:.2f} %'.format(np.nanmean(delta[:8,:])))
                print ('Decrease near surface: {:.2f} %'.format(np.nanmean(delta[:2,:])))
                print ('Right side: {:.2f} %'.format(np.nanmean(delta[:8,12:])))
                print ('left side: {:.2f} %'.format(np.nanmean(delta[:8,:12])))

                im2 = ax.imshow(delta,origin='lower',cmap=cmap2,vmin=-60,vmax=60)
...
axes.ravel()[2].set_ylabel('z(m)',fontsize=18)

 



cbar2 = fig.colorbar(im2, ax=axes.ravel().tolist(),orientation='horizontal',aspect=40)
cbar2.set_label('w (m/s)',rotation=0,fontsize=18)
cbar2.ax.tick_params(labelsize=16) 

[print ((v-vert[0])/vert[0]*100) for v in vert]
cbar2.set_label(r'$\Delta (\%)$',rotation=0,fontsize=18)

fig2,ax2 = plt.subplots(figsize=(12,8))
Ntot2 = Ntot_vals[-1]*1E-6
Ntot2 = np.mean(Ntot2[off:,c0:c00,r0:r00:step],axis=1)
delta = (Ntot2-base)/base*100.

im3 = ax2.imshow(delta,origin='lower',cmap=cmap2,vmin=-50,vmax=50)
ax2.set_ylabel('z(m)',fontsize=18)
ax2.set_xlabel('x(m)',fontsize=18)

ax2.set_xticks(labels[::3])
ax2.set_xticklabels(['{:.0f}'.format(s) for s in labels[::3]])

ax2.set_yticks(labels2[::2])
ax2.set_yticklabels(['{:.0f}'.format(2*s) for s in labels2[::2]])
